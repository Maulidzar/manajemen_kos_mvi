...
import com.example.kosmvvm.viewmodel.EditKamarIntent
...
class EditKamarActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (idKamar == -1) {
            ...
            isEditMode = true
            editKamarViewModel.dispatch(EditKamarIntent.LoadKamar(idKamar))
        ...
        lifecycleScope.launch {
            editKamarViewModel.state.collect { state ->
                val data = state.kamarData
                if (data != null && data != currentKamarData) {
                ...
                state.saveResult?.let { result ->
                    when (result) {
                        is SaveResult.Success -> {
                            Toast.makeText(this@EditKamarActivity, "Data Kamar Berhasil Disimpan!", Toast.LENGTH_SHORT).show()
                            finish()
                        }
                        is SaveResult.Error -> {
                            Toast.makeText(this@EditKamarActivity, result.message, Toast.LENGTH_LONG).show()
                            editKamarViewModel.dispatch(EditKamarIntent.ResetSaveStatus)
                        }
                    }
                }
            }
    ...
    private fun showSaveConfirmationDialog() {
        val dialogTitle = if (isEditMode) "Edit Kamar" else "Tambah Kamar Baru?"
    ...
    private fun saveChanges() {
        ...
        val kamar = KamarEntity(
            ...
            tanggalBayar = hariJatuhTempo,
            statusMaintenance = statusMaintenance,
            idPenghuni = data?.kamar?.idPenghuni
        )

        editKamarViewModel.dispatch(EditKamarIntent.SaveKamar(kamar, penghuni))
    }
